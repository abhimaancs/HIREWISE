...
function SignupForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error: profileError } = await supabase.from('profiles').upsert({ id: userId, role, name: form.name, email: form.email }, { onConflict: 'id' })
      if (profileError) throw new Error(profileError.message)
      if (role === 'candidate') {
        await supabase.from('candidate_profiles').upsert({ id: userId, college: form.college || '', skills: [], experience_years: 0, location: form.location || '' }, { onConflict: 'id' })
        router.push('/onboarding')
      } else {
        await supabase.from('company_profiles').upsert({ id: userId, company_name: form.company_name || form.name, location: form.location || '' }, { onConflict: 'id' })
